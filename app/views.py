...
def ami(account, region):
    c = connect(account,region, 'ec2')
    amis = c.describe_images(Owners=['self'])
    return render_template('ami.html', region=region, amis=amis)
...
def snapshots(account, region):
    c = connect(account, region, 'ec2')
    snapshots = c.describe_snapshots(OwnerIds=['self'])
...
def autoscale(account, region):
    ...
    asg = c.describe_auto_scaling_groups()
    pprint(asg)
...
def elasticache(account, region):
    ...
    clusters = c.describe_cache_clusters(ShowCacheNodeInfo=True)
    pprint(clusters)
    return render_template('elasticache.html', clusters=clusters)
...
def rds(account, region):
    ...
    db_instances = c.describe_db_instances()
...
def dynamodb(account, region):
    ...
    if tables:
        tables = [c.describe_table(TableName=x) for x in tables['TableNames']]
    else:
        tables = list()
